...
func (o *OpDesc) Name() string {
	if o.Op == OpCustom && o.Custom != "" {
		return "CUSTOM:" + o.Custom
	}
	return o.Op.String()
}
...
func Parse(b []byte) (m *Model, err error) {
	if len(b) < 8 {
		return nil, fmt.Errorf("%w: %w", ErrBadModel, errShort)
	}
	defer func() {
		if r := recover(); r != nil {
			m, err = nil, fmt.Errorf("%w: malformed flatbuffer: %v", ErrBadModel, r)
		} else if err != nil && !errors.Is(err, ErrBadModel) {
			m, err = nil, fmt.Errorf("%w: %w", ErrBadModel, err)
		}
	}()
	if string(b[4:8]) != "TFL3" {
		return nil, fmt.Errorf("%w: missing TFL3 identifier", ErrBadModel)
	}
	...
	for i, subs := 0, r.vec(modelSubgraphs); i < subs.len(); i++ {
		...
		for j, ts := 0, st.vec(subgraphTensors); j < ts.len(); j++ {
			...
			d := &TensorDesc{
				...
				Type:     TensorType(tt.u8f(tensorType)),
				Shape:    tt.vec(tensorShape).ints(),
				Buffer:   int(tt.u32f(tensorBuffer, 0)),
				Variable: tt.boolf(tensorIsVariable),
			}
		...
		for j, os := 0, st.vec(subgraphOperators); j < os.len(); j++ {
			...
			if code == 0 {
				...
				code = Op(ct.u8f(opcodeDeprecatedBuiltin))
			}
			o := &OpDesc{
				Op:      code,
...
func (m *Model) validate() error {
	for graphIndex, graph := range m.Subgraphs {
		if err := m.validateGraph(graph, graphIndex); err != nil {
			return err
		}
	}
	return nil
...
func (m *Model) validateGraph(graph *Subgraph, graphIndex int) error {
	if graph == nil {
		return fmt.Errorf("subgraph %d is nil", graphIndex)
	}
	for tensorIndex, tensor := range graph.Tensors {
		if tensor == nil || tensor.Buffer < 0 || tensor.Buffer >= len(m.buffers) {
			return fmt.Errorf("subgraph %d tensor %d has invalid buffer", graphIndex, tensorIndex)
		}
		for _, dimension := range tensor.Shape {
			if dimension < 0 {
				return fmt.Errorf("subgraph %d tensor %d has negative shape %v", graphIndex, tensorIndex, tensor.Shape)
			}
		}
	}
	for _, index := range append(append([]int(nil), graph.Inputs...), graph.Outputs...) {
		if index < 0 || index >= len(graph.Tensors) {
			return fmt.Errorf("subgraph %d references tensor %d of %d", graphIndex, index, len(graph.Tensors))
		}
	}
	for operatorIndex, operator := range graph.Ops {
		if err := validateOperator(operator, graphIndex, operatorIndex, len(graph.Tensors)); err != nil {
			return err
		}
	}
	return nil
...
func validateOperator(operator *OpDesc, graphIndex, operatorIndex, tensorCount int) error {
	if operator == nil || len(operator.Outputs) == 0 {
		return fmt.Errorf("subgraph %d operator %d has no output", graphIndex, operatorIndex)
	}
	minimum := minimumInputs(operator.Op)
	if len(operator.Inputs) < minimum {
		return fmt.Errorf("subgraph %d operator %d (%s) has %d inputs, need %d", graphIndex, operatorIndex, operator.Name(), len(operator.Inputs), minimum)
	}
	for _, index := range operator.Inputs {
		if index < -1 || index >= tensorCount {
			return fmt.Errorf("subgraph %d operator %d references input tensor %d", graphIndex, operatorIndex, index)
		}
	}
	for _, index := range operator.Outputs {
		if index < 0 || index >= tensorCount {
			return fmt.Errorf("subgraph %d operator %d references output tensor %d", graphIndex, operatorIndex, index)
		}
	}
	return nil
}

func minimumInputs(op Op) int {
	switch op {
